...
class UDPServer:
    ...
    def process_packet(self, packet, addr):
        ...
        try:
            ...
            self.gui.display_received_data(f"DEBUG: Total targets unpacked: {len(targets)}")
    ...
    def unpack_target_data_packet(self, data):
        ...
        # Unpack the target data header (frameID + packetNumber)
        ...
        for i in range(42):  # Maximum of 42 targets per packet
            target_start = 4 + i * 24  # Each target is 24 bytes, starting after the header
            if target_start + 24 > len(data):
                print(f"DEBUG: Not enough data for target {i + 1} at index {target_start}")
                break  # Stop if there isn't enough data for the next target
